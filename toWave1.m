...
cd(pathName);
fileName = '5146588 _박용복_L ADD LONGUS_N.txt';
...
for i=1:3000
     ...
     if (startsWith (currLine, 'Sweep  Data(mV)<960>=','IgnoreCase',true))
         currLine = currLine(startIndex+1:length(currLine));
         tSignal = [tSignal, currLine];
     elseif (startsWith (currLine, 'Trace Duration(ms)=','IgnoreCase',true))
         currLine = currLine (20:length(currLine));
         traceDuration = str2double(currLine)/1000;
     elseif (startsWith (currLine, 'Amplifier Range','IgnoreCase',true))
